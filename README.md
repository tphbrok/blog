...
It's a Rust program that converts a bunch of Markdown files (in `posts/`) to a bunch of HTML files (in `site/`, because GitHub Pages requires either `/` or `site/` as a source directory). I could've used some random existing static site generator, but where's the fun in that?!

## Why run a blog?

- To enrich the internet with some personal hand-made content, contrary to the ever-increasing blob of generated content
- To publicly practice and learn, and explicitly _not_ use AI in the process, since I'm doing this for learning purposes, not for the output
